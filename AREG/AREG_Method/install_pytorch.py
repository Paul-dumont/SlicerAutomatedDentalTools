...
def install_cuda_toolkit():
    ...
    logger.info("Installing CUDA toolkit via conda...")
    
    # Find conda executable
    conda_cmd = find_conda_executable()
    
    if not conda_cmd:
        logger.error("Cannot install CUDA toolkit - conda not found")
        return False
    
    try:
        # Get current environment name
        env_name = os.environ.get('CONDA_DEFAULT_ENV', 'base')
        logger.info(f"Installing CUDA toolkit in environment: {env_name}")
        ...
        if result.returncode == 0:
            ...
            logger.error("CUDA toolkit installation failed")
            return False
            
    except Exception as e:
        logger.error(f"CUDA toolkit installation failed: {e}")
        return False

def check_and_install_gcc():
    """Check GCC version and install newer version if needed"""
    logger.info("Checking GCC version...")
    
    try:
        result = run_command(["gcc", "--version"], check=False)
        if result.returncode == 0:
            version_line = result.stdout.split('\n')[0]
            logger.info(f"Current GCC: {version_line}")
            ...
            if match:
                ...
                if major >= 9:
                    logger.info(f"GCC {major} is sufficient")
                    return True
                else:
                    logger.warning(f"GCC {major} is too old, need GCC 9+")
                    return install_newer_gcc()
        
        logger.warning("Could not determine GCC version")
        return install_newer_gcc()
        
    except Exception as e:
        logger.warning(f"GCC check failed: {e}")
        return install_newer_gcc()

def install_newer_gcc():
    """Install newer GCC via conda"""
    logger.info("Installing newer GCC via conda...")
    
    conda_cmd = find_conda_executable()
    if not conda_cmd:
        logger.error("Cannot install GCC - conda not found")
        return False
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info("Newer GCC installed via conda")
            ...
            if gcc_path and gxx_path:
                ...
                logger.info(f"Set CC to: {gcc_path}")
                logger.info(f"Set CXX to: {gxx_path}")
                return True
            else:
                logger.warning("New GCC installed but not found in PATH")
                return False
        else:
            logger.error("GCC installation failed")
            return False
            
    except Exception as e:
        logger.error(f"GCC installation failed: {e}")
        return False
...
def check_and_fix_torchvision():
    ...
    try:
        ...
        logger.info("Checking torch/torchvision compatibility...")
        ...
        if expected_tv_version is None:
            logger.warning(f"Unknown PyTorch version {torch_version}, skipping compatibility check")
            return False
        
        if torchvision_version[:2] != expected_tv_version:
            logger.error(f"TorchVision {torchvision.__version__} is incompatible with PyTorch {torch.__version__}")
            logger.info(f"Expected TorchVision 0.{expected_tv_version[1]}.x for PyTorch {torch.__version__}")
            logger.info("Fixing torchvision installation...")
            ...
            if result.returncode == 0:
                logger.info(f"TorchVision fixed to {expected_tv_full}")
                return True
            else:
                logger.error("Failed to fix TorchVision")
                return False
        else:
            logger.info("PyTorch and TorchVision are compatible")
            return False
            
    except ImportError:
        logger.warning("Could not check torchvision compatibility")
        return False
    except Exception as e:
        logger.warning(f"Error checking torchvision: {e}")
        return False
...
def setup_cuda_environment():
    ...
    if nvcc_path:
        logger.info(f"Found nvcc at: {nvcc_path}")
    else:
        ...
        if cuda_installed:
            ...
            nvcc_path = shutil.which("nvcc")
            if nvcc_path:
                logger.info(f"Found nvcc after installation: {nvcc_path}")
            else:
                logger.warning("nvcc still not found after CUDA installation")
        else:
    ...
    if nvcc_path:
        ...
        logger.info(f"Set CUDA_HOME to: {cuda_home}")
        
        # Get CUDA version
        try:
            result = run_command(["nvcc", "--version"])
            logger.info(f"NVCC version info:\n{result.stdout}")
        except:
            pass
    else:
        logger.warning("Proceeding without CUDA toolkit")
        logger.warning("PyTorch3D compilation may fail or use CPU-only")
    ...
    logger.info(f"FORCE_CUDA: {os.environ.get('FORCE_CUDA')}")
    logger.info(f"TORCH_CUDA_ARCH_LIST: {os.environ.get('TORCH_CUDA_ARCH_LIST')}")
    
    return nvcc_path is not None

def install_pytorch3d():
    """Install PyTorch3D from GitHub"""
    logger.info("Installing PyTorch3D from GitHub...")
...
def verify_installation():
    ...
    logger.info("Verifying PyTorch3D installation...")
    # this is needed to check if compile with GPU support
    
    try:
        import torch
        import pytorch3d
        logger.info(f"PyTorch3D version: {pytorch3d.__version__}")
        ...
        logger.info("GPU operations successful!")
        logger.info(f"Sampled points shape: {points.shape}")
        logger.info(f"Points device: {points.device}")
        
        # Test renderer import
        import pytorch3d.renderer
        logger.info("PyTorch3D renderer import successful!")
...
def main():
    ...
    env_name = os.environ.get('CONDA_DEFAULT_ENV', 'base')
    logger.info(f"Installing in environment: {env_name}")

    
    try:
        # Install/verify PyTorch
        torch_version = install_pytorch_if_needed()
        logger.info(f"PyTorch {torch_version} ready")
        ...
        if gcc_ok:
            logger.info("GCC environment ready")
        else:
            logger.warning("GCC setup failed - compilation might fail")
        
        # Set up CUDA environment (includes auto-installation)
        cuda_available = setup_cuda_environment()
        
        if cuda_available:
            logger.info("CUDA environment ready")
        else:
            logger.warning("CUDA setup incomplete - GPU support may be limited")
