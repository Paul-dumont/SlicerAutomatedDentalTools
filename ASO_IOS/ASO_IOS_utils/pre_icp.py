...
def PrePreAso(source, target, landmarks):
    assert (
        len(landmarks) == 3 or len(landmarks) == 4
    ), f"landmark : {landmarks}, number : {len(landmarks)} "
    ...
    if len(landmarks) == 4:
        ...
        left_source, middle_source, right_source = (
            mean_source[left],
            np.mean(np.array([mean_source[middle[0]], mean_source[middle[1]]]), axis=0),
            mean_source[right],
        )
        left_target, middle_target, right_target = (
            mean_target[left],
            np.mean(np.array([mean_target[middle[0]], mean_target[middle[1]]]), axis=0),
            mean_target[right],
        )
        ...
        left_source, middle_source, right_source = (
            mean_source[left],
            mean_source[middle[0]],
            mean_source[right],
        )
        left_target, middle_target, right_target = (
            mean_target[left],
            mean_target[middle[0]],
            mean_target[right],
        )
    ...
    normal_target, direction_target = make_vector(
